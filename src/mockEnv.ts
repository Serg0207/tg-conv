// src/mockEnv.ts
// ЗАМЕНИТЕ ВЕСЬ код в этом файле на этот:
...
if (import.meta.env.DEV) {
  // Проверяем, запущены ли мы в браузере (не в Telegram)
  const isTelegramEnv = window.location.search.includes('tgWebAppData') 
    || window.location.hash.includes('tgWebAppData');

  // Если НЕ в Telegram - создаём mock окружение
  if (!isTelegramEnv) {
    const themeParams = {
    ...
    // Синхронная инициализация mock окружения
    mockTelegramEnv({
      onEvent(e) {
    ...
    console.info(
      '⚠️ As long as the current environment was not considered as the Telegram-based one, ' +
        'it was mocked. Take a note, that you should not do it in production and current ' +
        'behavior is only specific to the development process. Environment mocking is also ' +
        'applied only in development mode. So, after building the application, you will not ' +
        'see this behavior and related warning, leading to crashing the application outside ' +
        'Telegram.\n\n' +
        '✅ Mock environment initialized successfully!\n' +
        '💡 You can now develop locally without Telegram.',
    );
  }
}